            ...
            <div class="row">
                <div class="col-md-12">
                    <div class="card card-primary">
                        ...
                        <div class="card-body">
                            <form id="create-category" action="{{ route('admin.categories.categories-update', $category->id) }}" method="POST" enctype="multipart/form-data">
                                @csrf
                                <div class="form-group">
                                    ...
                                    <select name="parent" id="parent" class="form-control" disabled>
                                        ...
                                        @foreach($categories as $categoryItem)
                                            <option value="{{ $categoryItem->id }}" @if($categoryItem->id == $category->parent) selected @endif>{{ $categoryItem->title_default }}</option>
                                        @endforeach
                                    </select>
                                </div>
                                <div class="form-group">
                                    ...
                                    @foreach($languages as $language)
                                        <div class="tab-pane {{ $loop->first ? ' active' : '' }}" id="{{ $language->code }}" role="tabpanel">
                                            <div class="form-group">
                                                ...
                                                <input type="text" class="form-control" id="title-{{ $language->code }}" name="title[{{ $language->code }}]" value="{{ old('title.' . $language->code, optional(getCategoryDetailWithlanguageCode($category->id, $language->code))->title) }}" placeholder="Enter title">
                                                @if($errors->has('title'))
                                            ...
                                            <div class="form-group">
                                                ...
                                                <textarea class="form-control" id="description-{{ $language->code }}" name="description[{{ $language->code }}]" rows="3">{{ old('description.' . $language->code, optional(getCategoryDetailWithlanguageCode($category->id, $language->code))->description) }}</textarea>
                                        ...
                                        @foreach(config('constant.publish.text') as $key => $publish)
                                            <div class="radio mr-3 form-inline">
                                                <input class="mr-1" type="radio" name="publish" id="publish{{$key}}"
                                                       @if( $key == old('publish', $category->publish) ) checked @endif value="{{ $key }}">

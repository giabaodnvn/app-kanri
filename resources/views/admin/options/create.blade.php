            ...
            <div class="row">
                <div class="col-md-12">
                    <div class="card card-primary">
                        ...
                        <div class="card-body">
                            <form id="create-category" action="{{ route('admin.options.options-store') }}" method="POST">
                                @csrf
                                    ...
                                    @foreach($languages as $language)
                                        <li class="nav-item">
                                            <a class="nav-link {{ $loop->first ? ' active' : '' }}" data-toggle="tab"
                                               href="#{{ $language->code }}" role="tab">{{ $language->name }}</a>
                                        </li>
                                    @endforeach
                                </ul>
                                <!-- Tab panes -->
                                    ...
                                    @foreach($languages as $language)
                                        <div class="tab-pane {{ $loop->first ? ' active' : '' }}" id="{{ $language->code }}" role="tabpanel">
                                            <div class="form-group">
                                                ...
                                                <textarea class="form-control" id="content-{{ $language->code }}" name="content[{{ $language->code }}]" rows="3">{{ old('content.' . $language->code) }}</textarea>

            ...
            <div class="row">
                <div class="col-md-12">
                    <div class="card card-primary">
                        ...
                        <div class="card-body">
                            <form id="update-post" action="{{ route('admin.posts.posts-update', $post->id) }}" method="POST" enctype="multipart/form-data">
                                @csrf
                                @if($post->type == config('constant.post_type.post'))
                                    <div class="form-group">
                                        <label for="category_id">Category</label>
                                        <input type="text" class="form-control" value="{{ $post->category->title_default }}" disabled>
                                    </div>
                                @endif
                                <div class="form-group">
                                    <label>Image</label>
                                    ...
                                    @foreach($languages as $language)
                                        <div class="tab-pane {{ $loop->first ? ' active' : '' }}" id="{{ $language->code }}" role="tabpanel">
                                            <div class="form-group">
                                                ...
                                                <input type="text" class="form-control" id="title-{{ $language->code }}" name="title[{{ $language->code }}]" value="{{ old('title.' . $language->code, optional(getPostDetailWithlanguageCode($post->id, $language->code))->title) }}" placeholder="Enter title">
                                                @if($errors->has('title'))
                                            ...
                                            <div class="form-group">
                                                ...
                                                <textarea class="form-control" id="description-{{ $language->code }}" name="description[{{ $language->code }}]" rows="3">{{ old('description.' . $language->code, optional(getPostDetailWithlanguageCode($post->id, $language->code))->description) }}</textarea>
                                ...
                                <div class="form-group">
                                    ...
                                    <input type="text" class="form-control" id="link" name="link" value="{{ old('link', $post->link) }}" placeholder="Enter link">
                                    @if($errors->has('link'))
                                        ...
                                        @foreach(config('constant.publish.text') as $key => $publish)
                                            <div class="radio mr-3 form-inline">
                                                <input class="mr-1" type="radio" name="publish" id="publish{{$key}}"
                                                       @if( $key == old('publish', $post->publish) ) checked @endif value="{{ $key }}">
